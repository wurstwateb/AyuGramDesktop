...
void Session::checkTTLs() {
	...
	if (!expired.empty()) {
		auto toDestroy = expired | ranges::views::filter([](const auto item) {
			return !isMessageSavable(item);
		}) | ranges::to_vector;
		if (!toDestroy.empty()) {
...
void Session::processMessagesDeleted(
	...
	auto toProcess = std::vector<not_null<HistoryItem*>>();
	auto historiesToCheck = base::flat_set<not_null<History*>>();
	for (const auto &messageId : data) {
		const auto i = list ? list->find(messageId.v) : Messages::iterator();
		if (list && i != list->end()) {
			const auto history = i->second->history();
			toProcess.push_back(i->second);
	...
	if (!toProcess.empty()) {
		auto toDestroy = toProcess | ranges::views::filter([](const auto item) {
			return !isMessageSavable(item);
		}) | ranges::to_vector;
		if (!toDestroy.empty()) {
			notifyItemsAboutToBeDestroyed(toDestroy);
